...
app.get("/users/readall",auth,async function(req,res){
    const users = await dbo.collection("users").find().toArray();
    res.send({data:users,msg:"Success"});
});

app.post("/users/add",auth,async function(req,res){
    const user = req.body;
...
app.get("/users/delete/:id",auth,async function(req,res){
    try
    {
        ...
        const sid = new mongodb.ObjectId( req.params.id);
        const result  = await dbo.collection("users").deleteOne({_id:sid});
        const body = {};
        ...
        res.send({data:null,msg:err.message});
    }
})